...
class CacheManager:
        ...
        try:
            ...
            if cache_type == 'match_dicts':
                ...
                if len(parts) >= 3:  # league/year/page format
                    league = parts[0]
                    year = parts[1].replace('/', '_')  # Replace slashes in year with underscores
                    page = parts[2]
            ...
            self.logger.error(f"Error creating cache path for type '{cache_type}' and key '{key}': {str(e)}")
            raise

    def _is_cache_valid(self, cache_path: Path, cache_duration: Optional[timedelta] = None) -> bool:
    ...
    def _get(self, cache_type: str, key: str, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
        ...
        if self._is_cache_valid(cache_path, cache_duration):
            try:
                ...
                with open(cache_path, 'r') as f:
                    ...
                    self.logger.trace(f"Cache data content: {json.dumps(cache_data, indent=2)}")
                ...
                if cache_data.get('duration') is not None:
                    ...
                    if datetime.now() - cache_time > timedelta(seconds=cache_data['duration']):
                        self.logger.info(f"Cache expired for key: {key}")
                        return None
                    
                self.logger.debug(f"Successfully read data from cache: {cache_path}  with length: {len(cache_data['data'])}")
                return cache_data.get('data')
            except (json.JSONDecodeError, IOError) as e:
                self.logger.error(f"Failed to read cache file {cache_path}: {str(e)}")
                return None
        self.logger.info(f"Cache invalid or not found for key: {key}")
        return None

    def _save(self, cache_type: str, key: str, data: Any, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
        ...
        cache_path = self._get_cache_path(cache_type, key)
        try:
            self.logger.debug(f"Attempting to save data to cache with key: {key} and cache_type: {cache_type}")
            ...
            self.logger.info(f"Successfully saved data to cache: {cache_path}")
        except IOError as e:
    ...
    def get_match_dict(self, match_id: str, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
        """Get match data from cache"""
        return self._get('match_dict', match_id, cache_duration)

    def save_match_dict(self, match_id: str, match_data: Dict, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
        """Save match data to cache"""
        self._save('match_dict', match_id, match_data, cache_duration, url)

    # Match statistics methods
    def get_match_stats(self, match_id: str, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
        """Get match statistics from cache"""
        return self._get('match_stats', match_id, cache_duration)

    def save_match_stats(self, match_id: str, stats_data: Dict, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
        """Save match statistics to cache"""
        self._save('match_stats', match_id, stats_data, cache_duration, url)

    # Match odds methods
    def get_match_odds(self, match_id: str, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
        """Get match odds from cache"""
        return self._get('match_odds', match_id, cache_duration)

    def save_match_odds(self, match_id: str, odds_data: Dict, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
        """Save match odds to cache"""
        self._save('match_odds', match_id, odds_data, cache_duration, url)

    # Player IDs methods
    def get_player_ids(self, match_id: str, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
        """Get player IDs from cache"""
        return self._get('player_ids', match_id, cache_duration)

    def save_player_ids(self, match_id: str, player_data: Dict, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
        """Save player IDs to cache"""
        self._save('player_ids', match_id, player_data, cache_duration, url)

    # Positions methods
    def get_positions(self, key: str, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
        """Get positions from cache"""
        return self._get('positions', key, cache_duration)

    def save_positions(self, key: str, positions_data: Dict, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
        """Save positions to cache"""
        self._save('positions', key, positions_data, cache_duration, url)

    # Valid seasons methods
    def get_valid_seasons(self, league: str, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
        """Get valid seasons from cache"""
        return self._get('valid_seasons', league, cache_duration)

    def save_valid_seasons(self, league: str, seasons_data: Dict, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
        """Save valid seasons to cache"""
        self._save('valid_seasons', league, seasons_data, cache_duration, url)

    # Match dicts methods
    def get_match_dicts(self, league: str, year: str, page: int, cache_duration: Optional[timedelta] = None) -> Optional[Dict]:
    ...
    def save_match_dicts(self, league: str, year: str, page: int, matches_data: Dict, cache_duration: Optional[timedelta] = None, url: Optional[str] = None) -> None:
    ...
    def is_match_finished(self, match_id: str) -> bool:
        ...
        self.logger.debug(f"Checking if match {match_id} is finished: {len(match_data)}")
        if match_data:
