...
UNPROXIED_MAX_WINDOW = float(os.getenv('UNPROXIED_MAX_WINDOW', '2'))  # Hours before forcing proxy
UNPROXIED_LOCKOUT_WINDOW = float(os.getenv('UNPROXIED_LOCKOUT_WINDOW', '1'))  # Hours before allowing unproxied again
...
PROXY_PERCENTAGE = float(os.getenv('PROXY_PERCENTAGE', '100'))  # Default to 100% if not set
...
def botasaurus_get(req: Request, url: str) -> Response:
    ...
    if unproxied_lockout_until and current_time < unproxied_lockout_until:
        ...
        if not use_proxy:
            ...
            time_since_start = (current_time - unproxied_start_time).total_seconds() / 3600
            if time_since_start >= UNPROXIED_MAX_WINDOW:
                # Set lockout period
                unproxied_lockout_until = current_time + timedelta(hours=UNPROXIED_LOCKOUT_WINDOW)
                use_proxy = True
                logger.info(f"Maximum unproxied window ({UNPROXIED_MAX_WINDOW}h) reached. Enforcing proxy usage for {UNPROXIED_LOCKOUT_WINDOW}h")
    ...
    if not use_proxy:
        ...
        logger.info(f"Requesting (Unproxied): {url}")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            resp = req.get(url, proxies={'http': proxy, 'https': proxy}, headers=headers, verify=False)
            if resp.status_code == 404:
                logger.info(f"Resource not found (404) for URL: {url}")
                return resp
            if resp.status_code >= 200 and resp.status_code < 300:
